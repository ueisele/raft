...
func TestPartitionableDecorator(t *testing.T) {
	...
	if partition, ok := helpers.GetTransportCapability[transporttest.PartitionCapable](cluster, followerID); ok {
...
func TestPartitionGroups(t *testing.T) {
	...
	group1 := []int{0, 1}
	group2 := []int{2, 3, 4}
	if err := helpers.CreatePartition(cluster, group1, group2); err != nil {
		t.Fatalf("Failed to create partition: %v", err)
	}

	// Verify nodes in group1 cannot reach group2
	for _, id1 := range group1 {
		if partition, ok := helpers.GetTransportCapability[transporttest.PartitionCapable](cluster, id1); ok {
	...
	for _, id2 := range group2 {
		if partition, ok := helpers.GetTransportCapability[transporttest.PartitionCapable](cluster, id2); ok {
			for _, id1 := range group1 {
	...
	for i := 0; i < 5; i++ {
		if partition, ok := helpers.GetTransportCapability[transporttest.PartitionCapable](cluster, i); ok {
			for j := 0; j < 5; j++ {
